...
опциям (\inl{data}, \inl{methods}, \inl{computed}).  Это решает
...
Композабл — это функция, начинающаяся с \inl{use}, которая
...
\inl{setup()} или \inl{<script setup>}.  Это позволяет Vue
корректно привязать реактивные зависимости к текущему экземпляру
компонента.}

\important{Избегайте создания побочных эффектов в
\inl{computed()} — вычисляемые свойства должны быть чистыми
функциями.  Для побочных эффектов используйте \inl{watch()}
или \inl{watchEffect()}.}
