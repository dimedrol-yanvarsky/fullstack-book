...
Ключевое слово \texttt{async} превращает функцию в генератор промисов,
а \texttt{await} приостанавливает выполнение до разрешения промиса.

Под капотом среда исполнения использует конечный автомат: каждый
\texttt{await} создаёт точку приостановки, и функция возобновляется
...
Время выполнения последовательных \texttt{await} складывается линейно:
...
\note{Используйте \texttt{Promise.all} внутри \texttt{async}-функций,
...
\important{Не забывайте обрабатывать ошибки в \texttt{async}-функциях.
Необработанный \texttt{reject} в Node.js приводит к завершению процесса
начиная с версии~15.}
